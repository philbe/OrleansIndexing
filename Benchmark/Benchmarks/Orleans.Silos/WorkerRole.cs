...
namespace Orleans.Azure.Silos
{
    public class WorkerRole : RoleEntryPoint
    {
        ...
        public override bool OnStart()
        {
            ...
            diag("OrleansAzureSilos-OnStart called");

            diag("OrleansAzureSilos-OnStart Initializing config");

            // The following do usually significantly improve write throughput for Azure storage
            ...
            diag("OrleansAzureSilos-OnStart called base.OnStart ok=" + ok);
        ...
        {
            diag("OrleansAzureSilos-Run entry point called");

            diag("OrleansAzureSilos-OnStart Starting Orleans silo");

            var config = new ClusterConfiguration();

            var datacenter = Orleans.Benchmarks.Common.Config.GetDataCenter();

            if (datacenter.StartsWith("localhost"))
                datacenter = "local";

            config.LoadFromFile("OrleansConfiguration."+datacenter+".xml");
            ...
            diag("OrleansAzureSilos-OnStart Orleans silo started ok=" + ok);
        ...
        {
            diag("OrleansAzureSilos-OnStop called");
            ...
            RoleEnvironment.Changing -= RoleEnvironmentChanging;
            base.OnStop();
            diag("OrleansAzureSilos-OnStop finished");
        }
        ...
        private void Example_ConfigureExistingStorageProvider(ClusterConfiguration config)
        {
            ...
            const string myProviderName = "GlobalStore"; // what ever arbitrary name you want to give to your provider
