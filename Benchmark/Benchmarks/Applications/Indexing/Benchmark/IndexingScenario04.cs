...
{
    class IndexingScenario04 : IScenario
    {
        public class Params
        {
            ...
            public string indexType { get; set; } // possible values "NotIndexed", "AIndex", "IIndex", "DSMIndex"
        ...
        public IndexingScenario04()
        {
            throw new Exception("this should never be used... parameters are not set");
        }

        public IndexingScenario04(string json, int seed)
        {
        ...
        public string Name { get { return String.Format("IndexingScenario04_robots={0}_runtime={1}_grainCount={2}_distinctKeyCount={3}_indexType={4}", parameters.numRobots, parameters.runTimeSecs, parameters.grainCount, parameters.distinctKeyCount, parameters.indexType); } }
        ...
        public async Task<string> RobotInit(IRobotContext context, int robotnumber)
        {
            ...
            for (int i = (robotnumber * initsPerRobot); i < Math.Min(((robotnumber + 1) * initsPerRobot), parameters.grainCount); i++)
            {
                ...
                {
                    IPlayerGrain p1 = GrainClient.GrainFactory.GetGrain<IPlayerGrain>(i, "Orleans.Benchmarks.Indexing.Scenario04.PlayerGrain");
                ...
                {
                    IIndexedPlayerAIndexGrain p1 = GrainClient.GrainFactory.GetGrain<IIndexedPlayerAIndexGrain>(i, "Orleans.Benchmarks.Indexing.Scenario04.IndexedPlayerAIndexGrain");
                    initTasks.Add( p1.SetLocation(getRandomLocation(random)) );
                }
                else if (parameters.indexType.Equals("IIndex", StringComparison.OrdinalIgnoreCase))
                {
                    IIndexedPlayerIIndexGrain p1 = GrainClient.GrainFactory.GetGrain<IIndexedPlayerIIndexGrain>(i, "Orleans.Benchmarks.Indexing.Scenario04.IndexedPlayerIIndexGrain");
                    initTasks.Add( p1.SetLocation(getRandomLocation(random)) );
                }
                else if (parameters.indexType.Equals("DSMIndex", StringComparison.OrdinalIgnoreCase))
                {
                    IIndexedPlayerDSMIndexGrain p1 = GrainClient.GrainFactory.GetGrain<IIndexedPlayerDSMIndexGrain>(i, "Orleans.Benchmarks.Indexing.Scenario04.IndexedPlayerDSMIndexGrain");
                    initTasks.Add( p1.SetLocation(getRandomLocation(random)) );
                }
                else
                {
        ...
        public async Task<string> RobotMeasure(IRobotContext context, int robotnumber)
        {
            ...
            while (numRequests < parameters.runTimeSecs && robotTimer.ElapsedMilliseconds / 1000 < parameters.runTimeSecs)
            {
                ...
                try
                {
                    ...
                    if (parameters.indexType.Equals("NotIndexed", StringComparison.OrdinalIgnoreCase))
                    {
                        IPlayerGrain p1 = GrainClient.GrainFactory.GetGrain<IPlayerGrain>(nextGrain, "Orleans.Benchmarks.Indexing.Scenario04.PlayerGrain");
                        requestTimer.Start();
                    ...
                    {
                        IIndexedPlayerAIndexGrain p1 = GrainClient.GrainFactory.GetGrain<IIndexedPlayerAIndexGrain>(nextGrain, "Orleans.Benchmarks.Indexing.Scenario04.IndexedPlayerAIndexGrain");
                        requestTimer.Start();
                        while (!(await p1.SetLocation(getRandomLocation(random))))
                        {
                            numReattempts++; // means that AzureTable choked on etag because of concurrent update -> retry
                    ...
                    {
                        IIndexedPlayerIIndexGrain p1 = GrainClient.GrainFactory.GetGrain<IIndexedPlayerIIndexGrain>(nextGrain, "Orleans.Benchmarks.Indexing.Scenario04.IndexedPlayerIIndexGrain");
                        requestTimer.Start();
                        while (!(await p1.SetLocation(getRandomLocation(random))))
                        {
                            numReattempts++; // means that AzureTable choked on etag because of concurrent update -> retry
                    ...
                    {
                        IIndexedPlayerDSMIndexGrain p1 = GrainClient.GrainFactory.GetGrain<IIndexedPlayerDSMIndexGrain>(nextGrain, "Orleans.Benchmarks.Indexing.Scenario04.IndexedPlayerDSMIndexGrain");
                        requestTimer.Start();
                        while (!(await p1.SetLocation(getRandomLocation(random))))
                        {
                            numReattempts++; // means that AzureTable choked on etag because of concurrent update -> retry
